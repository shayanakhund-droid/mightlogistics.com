(function(){
  ...
  function resetForm(){
    $('carrierForm')?.reset();$('carrierForm').dataset.id='';$('carrierModalTitle').textContent='Add Carrier';$('carrierSaveMessage').textContent='';$('carrierLookupMessage').textContent='Enter an MC number to retrieve carrier information automatically.';$('carrier_status').value='active';$('carrier_insuranceStatus').value='unknown';updateInsuranceMessage();
  }
  ...
  function fillCarrier(c){
    ...
    $('carrier_legalName').value=c?.legal_name||'';$('carrier_dba').value=c?.dba_name||'';$('carrier_mc').value=c?.mc_number||'';$('carrier_dot').value=c?.dot_number||'';$('carrier_contact').value=c?.contact_name||'';$('carrier_phone').value=phoneFmt(c?.phone||'');$('carrier_email').value=c?.email||'';$('carrier_address1').value=c?.address_line1||'';$('carrier_address2').value=c?.address_line2||'';$('carrier_city').value=c?.city||'';$('carrier_state').value=c?.state||'';$('carrier_zip').value=c?.zip_code||'';$('carrier_insurance').value=c?.insurance_expiration||'';$('carrier_insuranceStatus').value=c?.insurance_status||'unknown';$('carrier_insuranceCompany').value=c?.insurance_company||'';$('carrier_policyNumber').value=c?.insurance_policy_number||'';$('carrier_insuranceEffective').value=c?.insurance_effective_date||'';$('carrier_coverage').value=c?.insurance_coverage??'';$('carrier_status').value=c?.status||'active';$('carrier_notes').value=c?.notes||'';$('carrier_lookupMc').value=c?.mc_number||'';$('carrierSaveMessage').textContent='';updateInsuranceMessage();
  }
  ...
  async function lookupCarrier(){
    ...
    try{
      ...
      const c=data.data;
      $('carrier_legalName').value=c.legal_name||'';$('carrier_dba').value=c.dba_name||'';$('carrier_mc').value=c.mc_number||mc;$('carrier_dot').value=c.dot_number||'';$('carrier_contact').value=c.contact_name||'';$('carrier_phone').value=phoneFmt(c.phone||'');$('carrier_email').value=c.email||'';$('carrier_address1').value=c.address_line1||'';$('carrier_address2').value=c.address_line2||'';$('carrier_city').value=c.city||'';$('carrier_state').value=c.state||'';$('carrier_zip').value=c.zip_code||'';$('carrier_insuranceStatus').value=c.insurance_status||'unknown';$('carrier_insuranceCompany').value=c.insurance_company||'';$('carrier_policyNumber').value=c.insurance_policy_number||'';$('carrier_insuranceEffective').value=c.insurance_effective_date||'';$('carrier_insurance').value=c.insurance_expiration||'';$('carrier_coverage').value=c.insurance_coverage??'';
      ...
      const notes=[];if(c.authority_status)notes.push(`Authority: ${c.authority_status}`);if(c.power_units!=null)notes.push(`Power Units: ${c.power_units}`);if(c.drivers!=null)notes.push(`Drivers: ${c.drivers}`);if(c.dotsearch_url)notes.push(`Source: ${c.dotsearch_url}`);$('carrier_notes').value=notes.join('\n');
  ...
  async function saveCarrier(e){
    e.preventDefault();const id=$('carrierForm').dataset.id||null;const legal=$('carrier_legalName').value.trim();if(!legal){$('carrierSaveMessage').textContent='Legal name is required.';return;}
