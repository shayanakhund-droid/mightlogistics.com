...
(function(){
  ...
  async function loadCRM(){
    ...
    try {
      const result = await Promise.race([
        crmDb.from('quote_requests').select('id,quote_number,customer_id,customer_name,company_name,email,phone,origin,destination,pickup_date,status,created_at,notes').order('created_at',{ascending:false}),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Request timed out after 10 seconds.')), 10000))
      ]);
      ...
      crmQuotes.forEach(q => {
        ...
        if(!map.has(key)) map.set(key,{id:q.customer_id||key,company_name:q.company_name||'Unnamed customer',contact_name:q.customer_name||'—',email:q.email||'',phone:q.phone||'',notes:q.notes||'',created_at:q.created_at,updated_at:q.created_at,derived:!q.customer_id});
      });
  ...
  function customerQuotes(c){return crmQuotes.filter(q=>c.derived?(!q.customer_id&&String(q.email||'').toLowerCase()===String(c.email||'').toLowerCase()):String(q.customer_id)===String(c.id));}
  function renderCustomers(){
    ...
    rows.innerHTML=filtered.map(c=>{const related=customerQuotes(c);const last=related[0]?.created_at||c.updated_at;return `<tr><td class="customer"><strong>${esc(c.company_name)}</strong><span>Added ${date(c.created_at)}</span></td><td>${esc(c.contact_name)}</td><td>${c.email?`<a href="mailto:${esc(c.email)}">${esc(c.email)}</a>`:'—'}</td><td>${c.phone?`<a href="tel:${esc(c.phone)}">${esc(c.phone)}</a>`:'—'}</td><td><strong>${related.length}</strong></td><td>${dateTime(last)}</td><td><button class="outline crm-customer" data-id="${esc(c.id)}">View</button></td></tr>`;}).join('');
  ...
  function openCustomer(id){
    const c=crmCustomers.find(x=>String(x.id)===String(id));if(!c)return;const related=customerQuotes(c);
    ...
    crm$('drawerBody').innerHTML=`<div class="customer-profile"><div class="profile-company">${esc(c.company_name)}</div><div class="profile-meta">Customer since ${date(c.created_at)}</div></div><div class="detail-grid customer-details">${detail('Primary contact',esc(c.contact_name))}${detail('Email',c.email?`<a href="mailto:${esc(c.email)}">${esc(c.email)}</a>`:'—')}${detail('Phone',c.phone?`<a href="tel:${esc(c.phone)}">${esc(c.phone)}</a>`:'—')}${detail('Total quotes',`<strong>${related.length}</strong>`)}${detail('Internal notes',esc(c.notes),true)}</div><div class="history-heading"><div class="kicker">ACTIVITY</div><h4>Quote History</h4></div><div class="history-list">${history}</div>`;openDrawer();document.querySelectorAll('.crm-history').forEach(b=>b.addEventListener('click',()=>openQuote(b.dataset.id)));
  }
  async function openQuote(id){const {data,error}=await crmDb.from('quote_requests').select('*').eq('id',id).maybeSingle();if(error||!data)return;currentQuote=data;crm$('drawerKicker').textContent='QUOTE REQUEST';crm$('drawerTitle').textContent=quoteNo(data.quote_number);crm$('quoteActions').classList.remove('hidden');crm$('detailStatus').value=data.status;crm$('internalNotes').value=data.internal_notes||'';crm$('saveMessage').textContent='';crm$('drawerBody').innerHTML=`<div class="detail-grid">${detail('Company',`<strong>${esc(data.company_name)}</strong>`)}${detail('Contact',`<strong>${esc(data.customer_name)}</strong>`)}${detail('Email',data.email?`<a href="mailto:${esc(data.email)}">${esc(data.email)}</a>`:'—')}${detail('Phone',data.phone?`<a href="tel:${esc(data.phone)}">${esc(data.phone)}</a>`:'—')}${detail('Origin',`<strong>${esc(data.origin)}</strong>`)}${detail('Destination',`<strong>${esc(data.destination)}</strong>`)}${detail('Pickup date',date(data.pickup_date))}${detail('Equipment',esc(data.equipment))}${detail('Commodity',esc(data.commodity))}${detail('Weight',data.weight_lbs?`${Number(data.weight_lbs).toLocaleString()} lbs`:'—')}${detail('Pieces',data.pieces?esc(data.pieces):'—')}${detail('Special requirements',esc(data.special_requirements),true)}${detail('Customer notes',esc(data.notes),true)}</div>`;openDrawer();}
  async function saveCRMQuote(){if(!currentQuote)return;crm$('saveMessage').textContent='Saving…';const {error}=await crmDb.from('quote_requests').update({status:crm$('detailStatus').value,internal_notes:crm$('internalNotes').value.trim(),updated_at:new Date().toISOString()}).eq('id',currentQuote.id);if(error){crm$('saveMessage').textContent='Could not save changes.';return;}crm$('saveMessage').textContent='Saved.';await loadCRM();setTimeout(closeDrawer,650);}
  function injectStyle(){if(document.getElementById('crmInjectedStyle'))return;const s=document.createElement('style');s.id='crmInjectedStyle';s.textContent='.panel-subtitle{margin:6px 0 0;color:#7b8794;font-size:12px}.customer-profile{padding:18px;background:linear-gradient(135deg,#f4f8fb,#fff);border:1px solid #e5ebf0;border-radius:12px;margin-bottom:22px}.profile-company{font-size:21px;font-weight:800;letter-spacing:-.03em;color:#142538}.profile-meta{font-size:11px;color:#84909d;margin-top:5px}.history-heading{margin-top:28px;border-top:1px solid #e7ebef;padding-top:22px}.history-heading h4{margin:5px 0 12px;font-size:17px}.history-list{display:flex;flex-direction:column;gap:8px}.history-row{width:100%;display:flex;justify-content:space-between;align-items:center;text-align:left;background:#fff;border:1px solid #e1e7ec;border-radius:10px;padding:13px 14px;cursor:pointer}.history-row:hover{border-color:#9bbbd6;background:#f8fbfd}.history-row span{display:flex;flex-direction:column;gap:4px}.history-row strong{font-size:12px;color:#176fbe}.history-row small{font-size:10px;color:#7f8c99}.history-row .status{align-self:flex-end}.history-empty{padding:25px}.crm-customer{white-space:nowrap}';document.head.appendChild(s);}
  function init(){injectStyle();document.querySelectorAll('nav a[data-section]').forEach(a=>a.addEventListener('click',e=>{e.preventDefault();showSection(a.dataset.section);}));crm$('customerSearch')?.addEventListener('input',renderCustomers);crm$('customerRefresh')?.addEventListener('click',loadCRM);crm$('drawerClose')?.addEventListener('click',closeDrawer);crm$('drawerX')?.addEventListener('click',closeDrawer);crm$('saveQuote')?.addEventListener('click',()=>{if(currentQuote)saveCRMQuote();});if(location.hash==='#customers')showSection('customers');}
