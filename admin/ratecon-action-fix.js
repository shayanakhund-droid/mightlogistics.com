(function(){
  ...
  async function populateBrokerSelect(preserve=true){
    ...
    const {data,error}=await db.from('employee_profiles')
      .select('id,full_name,email,phone,access_level,is_active')
      .in('access_level',['administrator','broker'])
      .eq('is_active',true)
      .order('full_name');
    if(error){console.error('Broker list load failed:',error);return;}
    const options=(data||[]).map(p=>`<option value="${esc(p.id)}">${esc(p.full_name||p.email||'Broker')} (${p.access_level==='administrator'?'administrator':'broker'})</option>`).join('');
    s.innerHTML='<option value="">Unassigned</option>'+options;
  ...
  async function hydrateBroker(){
    ...
    await populateBrokerSelect(false);
  ...
  async function persistBrokerAssignment(){
    ...
    const {error}=await db.from('loads').update({
      assigned_employee:brokerId,
      updated_at:new Date().toISOString()
    }).eq('id',existingId);
    if(error){
      console.error('Broker assignment save failed:',error);
      if($('loadSaveMessage'))$('loadSaveMessage').textContent=`Broker assignment could not be saved: ${error.message}`;
    }
  }
  ...
  function ensureBrokerField(){
    ...
    const s=$('load_brokerId');
    if(s&&!s.dataset.brokerBound){
      s.dataset.brokerBound='1';
      s.addEventListener('change',()=>{ brokerDirty=true; });
    }
    if(!s?.dataset.brokerPopulated){
      s.dataset.brokerPopulated='1';
      populateBrokerSelect(false);
    }
  }

  async function onLoadOpened(){
    brokerDirty=false;
    ensureBrokerField();
    await populateBrokerSelect(false);
    await hydrateBroker();
  ...
  function showBrokerOnButton(){
    const form=$('loadForm'),b=$('rateconGenerateAction');
    if(!form||!b)return;
  ...
  async function saveOperational(load){
    ...
    const {data,error}=await db.from('loads').update(payload).eq('id',load.id).select('*').single();
    if(error)throw error;
    return data;
  }

  function replaceBrokerBlock(html,broker){
    if(!broker)return html;
    const name=esc(broker.full_name||broker.email||'—');
    const email=esc(broker.email||'—');
    const phone=esc(broker.phone||'—');
    return html
      .replace(/<span>Broker Name<\/span><b>.*?<\/b>/,'<span>Broker Name</span><b>'+name+'</b>')
      .replace(/<span>Broker Email<\/span><b>.*?<\/b>/,'<span>Broker Email</span><b>'+email+'</b>')
      .replace(/<span>Broker Phone<\/span><b>.*?<\/b>/,'<span>Broker Phone</span><b>'+phone+'</b>')
      .replace(/<span>Broker<\/span><b>Might Logistics<\/b>/,'<span>Broker</span><b>Might Logistics</b>');
  }

  async function generateAndEmail(){
    const b=$('rateconGenerateAction');
    let rateTab=null,mailTab=null;
    try{
      ...
      if(selectedBroker!==load.assigned_employee){
      ...
      load=await saveOperational(load);
      load=await getLoad();
      const broker=await getAssignedBroker(load);
      if(!broker)throw new Error('Select a Broker / Account Executive before generating the rate confirmation.');
      if(!window.mightRateCon?.rateConHtml)throw new Error('Rate confirmation module is not loaded. Refresh the page and try again.');

      let html=window.mightRateCon.rateConHtml(load,broker);
      html=replaceBrokerBlock(html,broker);
      if(!html||html.length<1000)throw new Error('Rate confirmation document was empty.');

      const blob=new Blob([html],{type:'text/html;charset=utf-8'});
      const {data:existing,error:findError}=await db.from('load_documents')
        .select('id').eq('load_id',load.id).eq('document_type','rate_confirmation')
      ...
      const payload={
        ...
        file_name:`${load.load_number}_Rate_Confirmation.html`,
        mime_type:'text/html',
        file_size:blob.size,
        storage_path:null,
        content:html,
      ...
      if(existing?.id){
        const {error}=await db.from('load_documents').update(payload).eq('id',existing.id);
        if(error)throw error;
      }else{
      ...
      if(carrierError)console.warn(carrierError);
      email=carrier?.email||'';
      if(email){
        ...
        mailTab.document.open();mailTab.document.write('<div style="font-family:Arial;padding:40px"><h2>Rate Confirmation Generated</h2><p>No carrier email address is saved.</p></div>');mailTab.document.close();
      }
    }catch(err){
      console.error(err);
      if(rateTab&&!rateTab.closed){rateTab.document.open();rateTab.document.write('<div style="font-family:Arial;padding:40px;color:#b42318"><h2>Rate Confirmation could not be generated</h2><p>'+esc(err.message||err)+'</p></div>');rateTab.document.close();}
      if(mailTab&&!mailTab.closed){mailTab.document.open();mailTab.document.write('<div style="font-family:Arial;padding:40px;color:#b42318"><h2>Email could not be prepared</h2><p>'+esc(err.message||err)+'</p></div>');mailTab.document.close();}
      alert(`Rate confirmation failed: ${err.message||err}`);
    }finally{
      if(b){b.disabled=false;b.textContent='Generate Rate Confirmation & Email';showBrokerOnButton();}
    }
  }

  function ensureButton(){
    ensureBrokerField();
    const form=$('loadForm'),actions=form?.querySelector('.loads-form-actions');
    if(!form||!actions)return;
    let b=$('rateconGenerateAction');
    if(!b){
      b=document.createElement('button');
      b.type='button';b.id='rateconGenerateAction';b.className='primary';
      b.textContent='Generate Rate Confirmation & Email';b.style.display='none';
      actions.insertBefore(b,$('loadModalClose2'));
      b.addEventListener('click',generateAndEmail);
    }
    showBrokerOnButton();
  }
  ...
  function init(){
    ...
    document.addEventListener('click',e=>{
      if(e.target?.id==='createLoad'){
        setTimeout(async()=>{brokerDirty=false;ensureBrokerField();await populateBrokerSelect(false);const s=$('load_brokerId');if(s)s.value='';},100);
      }
      if(e.target?.closest?.('.load-view')){
        setTimeout(onLoadOpened,150);
      }
    },true);

    $('load_status')?.addEventListener('change',showBrokerOnButton);
    $('loadForm')?.addEventListener('submit',()=>{if($('load_brokerId'))brokerDirty=true;},true);
