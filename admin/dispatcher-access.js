(function(){
  ...
  async function apply(){
    try{
      ...
      const isDispatcher=p?.access_level==='dispatcher';
      window.mightDispatcherRestricted=isDispatcher;
      document.querySelectorAll('nav a[data-section]').forEach(a=>{a.style.display=(isDispatcher&&a.dataset.section!=='dispatch')?'none':'';});
      const dash=document.getElementById('dashboard');
      if(isDispatcher&&dash)dash.classList.add('hidden');
      const title=document.getElementById('pageTitle');
      if(isDispatcher&&title)title.textContent='Dispatch Operations';
