(function(){
  ...
  function patchBlob(){
    ...
    class BrokerBlob extends NativeBlob{
      constructor(parts,options){
        try{
          if(profile&&profileLoadId===loadId()&&Array.isArray(parts))parts=parts.map(x=>typeof x==='string'?decorate(x):x);
        }catch(e){console.warn('Rate confirmation Blob broker patch failed:',e)}
        super(parts,options);
      }
    }
  ...
  function init(){
    ...
    setInterval(()=>{patchBlob();patchDocuments();if(!$('loadModal')?.classList.contains('hidden')&&loadId())refreshProfile()},500);
  }
