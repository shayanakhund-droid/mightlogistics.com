(function(){
  function loadScript(src,key){
    ...
    window[key]=true;
    return new Promise(resolve=>{const s=document.createElement('script');s.src=src;s.onload=()=>resolve();s.onerror=()=>{console.error('Could not load '+src);resolve()};document.head.appendChild(s)});
  }
  function loadEnhancements(){return loadScript('enhancements.js?v=8','mightEnhancementsLoaded')}
  function loadDashboardV2(){return loadScript('dashboard-v2.js?v=1','mightDashboardV2Loaded')}
  ...
  async function init(){await loadEnhancements();await loadDashboardV2();moveQuotesOutOfDashboard();document.addEventListener('click',function(e){const link=e.target.closest?.('nav a[data-section]');if(!link)return;e.preventDefault();e.stopImmediatePropagation();showSection(link.dataset.section,true)},true);const initial=(location.hash||'#dashboard').slice(1);showSection(document.getElementById(initial)?initial:'dashboard',false);window.mightAdminRouter={showSection}}
